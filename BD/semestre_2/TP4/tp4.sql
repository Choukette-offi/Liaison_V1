...
WITH DixMars2024 AS (
    SELECT P.refProd, P.nomProd, P.PUProd
    FROM PRODUIT P
    JOIN DETAIL D ON P.refProd = D.refProd
    JOIN FACTURE F ON D.numFac = F.numFac
    WHERE F.dateFac = STR_TO_DATE('10-03-2024', '%d-%m-%Y')
)
...
WITH NbClientsParVille AS (
    SELECT DISTINCT adresseCli AS ville, COUNT(*) 
    from CLIENT
...
from PRODUIT
LEFT JOIN DETAIL
ON PRODUIT.refProd = DETAIL.refProd
