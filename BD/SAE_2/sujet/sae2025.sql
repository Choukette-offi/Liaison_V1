...
WITH VentesParAuteur AS (
    SELECT 
        YEAR(datecom) AS annee,
        nomauteur,
        SUM(qte) AS total_vendus
    FROM DETAILCOMMANDE
...
SELECT annee, nomauteur, total_vendus
FROM VentesParAuteur
WHERE (annee, total_vendus) IN (
    SELECT annee, MAX(total_vendus)
    FROM VentesParAuteur
    GROUP BY annee
)
ORDER BY annee;
